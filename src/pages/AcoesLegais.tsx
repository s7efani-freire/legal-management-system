...
import { Plus, Info, Trash2, Edit, Clock, FileText, UserCircle2 } from "lucide-react"; // Adicionado Edit e outros ícones
import DetailsPopup, { DetailItem } from "../components/ui/DetailsPopup";
import EditActionPopup from "../components/ui/EditActionPopup"; // Importa o novo popup de edição

// --- DEFINIÇÃO DE TIPO (Exportar para ser usado no EditActionPopup) ---
...
const AcoesLegais: React.FC = () => {
    // Estado dos dados (você substituirá por uma chamada de API no futuro)
    ...
    const handleOpenDetailsPopup = (id: number) => {
        ...
        if (acaoSelecionada) {
            const detailsList: DetailItem[] = [
                { icon: <FileText />, label: 'Tipo de Ação', value: acaoSelecionada.tipo },
                { icon: <UserCircle2 />, label: 'Responsável', value: acaoSelecionada.responsavel },
                { icon: <FileText />, label: 'Nº do Processo', value: acaoSelecionada.numeroProcesso },
                { icon: <Clock />, label: 'Criado em', value: new Date(acaoSelecionada.criadoEm).toLocaleDateString('pt-BR') },
                { label: 'Descrição', value: acaoSelecionada.descricao, isFullWidth: true },
            ];
    ...
    // Efeito para abrir popup vindo de uma notificação
    useEffect(() => {
                                ...
                                <tr key={item.id} className="border-t border-gray-200 hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-3 text-sm text-gray-700">
                                        <div className="flex items-center justify-center space-x-3">
                                            <button onClick={() => handleOpenDetailsPopup(item.id)} className="text-blue-500 hover:text-blue-700"><Info className="w-5 h-5" /></button>
                                            <button onClick={() => handleOpenEditPopup(item.id)} className="text-green-600 hover:text-green-800"><Edit className="w-5 h-5" /></button>
